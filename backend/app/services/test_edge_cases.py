...
from app.services.classification import classify_all, ExceptionSubtype, ReconStatus
from app.services.matching import MatchResult, detect_duplicate_orders, detect_ambiguous_bank_matches, ScoreBreakdown
...
def test_detect_duplicate_orders() -> None:
    orders = [
        ...
        NormalisedOrder("ORD1", datetime.date(2024, 1, 2), "A", Decimal("100"), "INR"), # Duplicate
    ]
...
def test_detect_ambiguous_bank_matches() -> None:
    ...
    r1 = MatchResult(
        ...
        composite_score=0.9,
        score_breakdown=ScoreBreakdown(reference_score=1.0, amount_score=0.9, date_score=0.9, bank_amount_score=1.0, bank_utr_score=1.0, amount_diff_inr=Decimal("0.0"), date_diff_days=0, best_utr_ratio=100.0)
    )
    r2 = MatchResult(
        order_id="ORD2",
...
def test_classification_overrides() -> None:
    ...
    r1 = MatchResult(
        ...
        score_breakdown=ScoreBreakdown(reference_score=1.0, amount_score=1.0, date_score=1.0, bank_amount_score=1.0, bank_utr_score=1.0, amount_diff_inr=Decimal("0.0"), date_diff_days=0, best_utr_ratio=100.0)
    )
...
def test_refund_mismatch_classification() -> None:
    ...
    r1 = MatchResult(
        ...
        matched_bank_utr="UTR123",
        composite_score=0.8,
        score_breakdown=ScoreBreakdown(reference_score=1.0, amount_score=0.5, date_score=1.0, bank_amount_score=1.0, bank_utr_score=1.0, amount_diff_inr=Decimal("200.0"), date_diff_days=0, best_utr_ratio=100.0)
    )
